...
# deployment
deploy-local: 
	@forge script script/DeployAutoRevToken.s.sol:DeployAutoRevToken --rpc-url $(RPC_LOCALHOST) --private-key ${DEFAULT_ANVIL_KEY} --sender ${DEFAULT_ANVIL_ADDRESS} --broadcast 

deploy-testnet: 
	@forge script script/DeployAutoRevToken.s.sol:DeployAutoRevToken --rpc-url $(RPC_TEST) --account ${ACCOUNT_NAME} --sender ${ACCOUNT_ADDRESS} --broadcast --verify --etherscan-api-key ${ETHERSCAN_KEY} -vvvv

deploy-mainnet: 
	@forge script script/DeployAutoRevToken.s.sol:DeployAutoRevToken --rpc-url $(RPC_MAIN) --account ${ACCOUNT_NAME} --sender ${ACCOUNT_ADDRESS} --broadcast --verify --verifier-url "https://api.routescan.io/v2/network/mainnet/evm/43114/etherscan" --etherscan-api-key ${ETHERSCAN_KEY} -vvvv

verify:
	@forge verify-contract 0xd86b56076b5ed2f31B8C34047E275d1C756a3783 src/AutoRevToken.sol:AutoRevToken --verifier-url "https://api.routescan.io/v2/network/mainnet/evm/43114/etherscan" --etherscan-api-key ${ETHERSCAN_KEY} --num-of-optimizations 200 --compiler-version v0.8.20+commit.a1b79de6 --constructor-args 
